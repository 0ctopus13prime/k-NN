...
#include "partial_loading_index_flat.h"
#include "partial_loading_index_flat_codes.h"
#include "partial_loading_index_hnsw.h"
...
namespace knn_jni::partial_loading {
  thread_local std::optional<knn_jni::stream::NativeEngineIndexInputMediator> PartialLoadingContext::mediator{};
...
std::unique_ptr<faiss::IDGrouperBitmap> buildIDGrouperBitmap(knn_jni::JNIUtilInterface *jniUtil,
                                                             JNIEnv *env,
...
void knn_jni::faiss_wrapper::InsertToIndex(knn_jni::JNIUtilInterface *jniUtil,
                                           JNIEnv *env,
                                           jintArray idsJ,
...
void knn_jni::faiss_wrapper::CreateBinaryIndexFromTemplate(knn_jni::JNIUtilInterface *jniUtil,
                                                           JNIEnv *env,
                                                           jintArray idsJ,
                                                           jlong vectorsAddressJ,
                                                           jint dimJ,
...
void knn_jni::faiss_wrapper::CreateByteIndexFromTemplate(knn_jni::JNIUtilInterface *jniUtil,
                                                         JNIEnv *env,
                                                         jintArray idsJ,
                                                         jlong vectorsAddressJ,
                                                         jint dimJ,
...
jlong knn_jni::faiss_wrapper::LoadIndex(knn_jni::JNIUtilInterface *jniUtil, JNIEnv *env, jstring indexPathJ) {
  ...
  faiss::Index *indexReader = faiss::read_index(indexPathCpp.c_str(),
                                                faiss::IO_FLAG_READ_ONLY | faiss::IO_FLAG_PQ_SKIP_SDC_TABLE
...
faiss::Index *read_index_opensearch(knn_jni::stream::FaissOpenSearchIOReader *f,
  ...
  if (h == faiss::fourcc("IxMp")) {
    ...
    idxf->codes.setMinimumItemsToLoad(sizeof(float) * idxf->d);
    ...
    throw std::runtime_error("NOOOOOOOOOOOOOO");
  }
...
jlong knn_jni::faiss_wrapper::LoadBinaryIndex(knn_jni::JNIUtilInterface *jniUtil, JNIEnv *env, jstring indexPathJ) {
  ...
  // Skipping IO_PRECOMPUTE_TABLE because it is only needed for IVFPQ-l2 and it leads to high memory consumption if
  // done for each segment. Instead, we will set it later on with `setSharedIndexState`
  faiss::IndexBinary *indexReader = faiss::read_index_binary(indexPathCpp.c_str(),
                                                             faiss::IO_FLAG_READ_ONLY | faiss::IO_FLAG_PQ_SKIP_SDC_TABLE
                                                                 | faiss::IO_FLAG_SKIP_PRECOMPUTE_TABLE);
  return (jlong) indexReader;
}
...
jobjectArray knn_jni::faiss_wrapper::QueryIndex(knn_jni::JNIUtilInterface *jniUtil,
                                                JNIEnv *env,
                                                jlong indexPointerJ,
                                                ...
                                                jintArray parentIdsJ) {
  return knn_jni::faiss_wrapper::QueryIndex_WithFilter(jniUtil,
                                                       env,
                                                       indexPointerJ,
                                                       partial_loading_ctx_j,
                                                       queryVectorJ,
                                                       kJ,
                                                       methodParamsJ,
                                                       nullptr,
...
//jobjectArray knn_jni::faiss_wrapper::QueryIndex_WithFilter(knn_jni::JNIUtilInterface *jniUtil,
//                                                           JNIEnv *env,
//                                                           jlong indexPointerJ,
//                                                           jfloatArray queryVectorJ,
//                                                           jint kJ,
...
//      hnswParams.efSearch =
//          knn_jni::commons::getIntegerMethodParameter(env, jniUtil, methodParams, EF_SEARCH, hnswReader->hnsw.efSearch);
...
//  // index
//  int32_t resultSize = kJ;
//  if (ids.back() == -1) {
//    if (ids.front() != -1) {
//      int32_t i = ids.size() - 1;
//      while (i >= 0 && ids[i] == -1) {
//        --i;
//      }
//      resultSize = i + 1;
...
jobjectArray knn_jni::faiss_wrapper::QueryIndex_WithFilter(knn_jni::JNIUtilInterface *jniUtil,
  ...
  jniUtil->HasExceptionInStack(env, "!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!! - 3");

  // If there are not k results, the results will be padded with -1.
  // Find the first -1, and set result size to that index
  int32_t resultSize;
  {
    int32_t i = 0;
    while (i < kJ && ids[i] != -1) {
  ...
  // Set results into KNNQueryResult[]
  jclass resultClass = jniUtil->FindClass(env, "org/opensearch/knn/index/query/KNNQueryResult");
  jmethodID allArgs = jniUtil->FindMethod(env, "org/opensearch/knn/index/query/KNNQueryResult", "<init>");
...
jobjectArray knn_jni::faiss_wrapper::QueryBinaryIndex_WithFilter(knn_jni::JNIUtilInterface *jniUtil,
                                                                 JNIEnv *env,
                                                                 jlong indexPointerJ,
                                                                 jbyteArray queryVectorJ,
                                                                 jint kJ,
                                                                 jobject methodParamsJ,
                                                                 jlongArray filterIdsJ,
                                                                 jint filterIdsTypeJ,
  ...
  if (filterIdsJ != nullptr) {
    ...
    if (ivfReader) {
      ...
      if (hnswReader != nullptr && (methodParamsJ != nullptr || parentIdsJ != nullptr)) {
        // Query param efsearch supersedes ef_search provided during index setting.
        hnswParams.efSearch = knn_jni::commons::getIntegerMethodParameter(env,
                                                                          jniUtil,
                                                                          methodParams,
                                                                          EF_SEARCH,
                                                                          hnswReader->hnsw.efSearch);
        if (parentIdsJ != nullptr) {
          idGrouper = buildIDGrouperBitmap(jniUtil, env, parentIdsJ, &idGrouperBitmap);
          hnswParams.grp = idGrouper.get();
        }
...
jbyteArray knn_jni::faiss_wrapper::TrainBinaryIndex(knn_jni::JNIUtilInterface *jniUtil,
                                                    JNIEnv *env,
                                                    jobject parametersJ,
                                                    jint dimensionJ,
...
std::unique_ptr<faiss::IDGrouperBitmap> buildIDGrouperBitmap(knn_jni::JNIUtilInterface *jniUtil,
  ...
  std::unique_ptr<faiss::IDGrouperBitmap>
      idGrouper = faiss_util::buildIDGrouperBitmap(parentIdsArray, parentIdsLength, bitmap);
...
jobjectArray knn_jni::faiss_wrapper::RangeSearch(knn_jni::JNIUtilInterface *jniUtil,
                                                 JNIEnv *env,
                                                 jlong indexPointerJ,
                                                 jfloatArray queryVectorJ,
                                                 jfloat radiusJ,
                                                 ...
                                                 jintArray parentIdsJ) {
  return knn_jni::faiss_wrapper::RangeSearchWithFilter(jniUtil,
                                                       env,
                                                       indexPointerJ,
                                                       queryVectorJ,
                                                       radiusJ,
...
jobjectArray knn_jni::faiss_wrapper::RangeSearchWithFilter(knn_jni::JNIUtilInterface *jniUtil,
                                                           JNIEnv *env,
                                                           jlong indexPointerJ,
                                                           jfloatArray queryVectorJ,
                                                           jfloat radiusJ,
