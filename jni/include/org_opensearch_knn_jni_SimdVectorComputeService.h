/* DO NOT EDIT THIS FILE - it is machine generated */
#include <jni.h>
/* Header for class org_opensearch_knn_jni_SimdVectorComputeService */

#ifndef _Included_org_opensearch_knn_jni_SimdVectorComputeService
#define _Included_org_opensearch_knn_jni_SimdVectorComputeService
#ifdef __cplusplus
extern "C" {
#endif
/*
 * Class:     org_opensearch_knn_jni_SimdVectorComputeService
 * Method:    bulkDistanceCalculation
 * Signature: ([I[FI)F
 */
JNIEXPORT jfloat JNICALL Java_org_opensearch_knn_jni_SimdVectorComputeService_scoreSimilarityInBulk
  (JNIEnv *, jclass, jintArray, jfloatArray, jint);

/*
 * Class:     org_opensearch_knn_jni_SimdVectorComputeService
 * Method:    saveSearchContext
 * Signature: ([F[JI)V
 */
JNIEXPORT void JNICALL Java_org_opensearch_knn_jni_SimdVectorComputeService_saveSearchContext
  (JNIEnv *, jclass, jfloatArray, jlongArray, jint);

/*
 * Class:     org_opensearch_knn_jni_SimdVectorComputeService
 * Method:    scoreSingleVector
 * Signature: (JI)F
 */
JNIEXPORT jfloat JNICALL Java_org_opensearch_knn_jni_SimdVectorComputeService_scoreSimilarity
  (JNIEnv *, jclass, jint);

/*
 * Class:     org_opensearch_knn_jni_SimdVectorComputeService
 * Method:    saveSQSearchContext
 * Signature: ([BFFFI[JIIF)V
 */
JNIEXPORT void JNICALL Java_org_opensearch_knn_jni_SimdVectorComputeService_saveSQSearchContext
  (JNIEnv *, jclass, jbyteArray, jfloat, jfloat, jfloat, jint, jlongArray, jint, jint, jfloat);

/*
 * Class:     org_opensearch_knn_jni_SimdVectorComputeService
 * Method:    saveSQSearchContext2
 * Signature: ([BFFFIIIF)V
 */
JNIEXPORT void JNICALL Java_org_opensearch_knn_jni_SimdVectorComputeService_saveSQSearchContext2
  (JNIEnv *, jclass, jbyteArray, jfloat, jfloat, jfloat, jint, jint, jint, jfloat);

/*
 * Class:     org_opensearch_knn_jni_SimdVectorComputeService
 * Method:    scoreSimilarityInBulk2
 * Signature: ([B[FI)F
 */
JNIEXPORT jfloat JNICALL Java_org_opensearch_knn_jni_SimdVectorComputeService_scoreSimilarityInBulk2
  (JNIEnv *, jclass, jbyteArray, jfloatArray, jint);

/*
 * Class:     org_opensearch_knn_jni_SimdVectorComputeService
 * Method:    scoreSimilarity2
 * Signature: ([B)F
 */
JNIEXPORT jfloat JNICALL Java_org_opensearch_knn_jni_SimdVectorComputeService_scoreSimilarity2
  (JNIEnv *, jclass, jbyteArray);

#ifdef __cplusplus
}
#endif
#endif
