...
public class BBQReader extends FlatVectorsReader {
    private static final long SHALLOW_SIZE = RamUsageEstimator.shallowSizeOfInstance(BBQReader.class);
    ...
    private final Map<String, BBQReader.FieldEntry> fields = new HashMap<>();
    private final IndexInput quantizedVectorData;
    private final Lucene102BinaryFlatVectorsScorer vectorScorer;

    public BBQReader(SegmentReadState state, Lucene102BinaryFlatVectorsScorer vectorsScorer) throws IOException {
        super(vectorsScorer);
        ...
        try (ChecksumIndexInput meta = state.directory.openChecksumInput(metaFileName)) {
            ...
            quantizedVectorData = openDataInput(
                state, versionMeta, VECTOR_DATA_EXTENSION, VECTOR_DATA_CODEC_NAME,
    ...
    private static IndexInput openDataInput(
        ...
        try {
            int versionVectorData =
                CodecUtil.checkIndexHeader(in, codecName, VERSION_START, VERSION_CURRENT, state.segmentInfo.getId(), state.segmentSuffix);
    ...
    public static final class BinarizedVectorValues extends FloatVectorValues {
        ...
        @Override
        public int size() {
            return quantizedVectorValues.size();
        }
        ...
        public Bits getAcceptOrds(Bits acceptDocs) {
            return quantizedVectorValues.getAcceptOrds(acceptDocs);
        }

        @Override
        public int ordToDoc(int ord) {
            return quantizedVectorValues.ordToDoc(ord);
        }

        @Override
        public DocIndexIterator iterator() {
            return quantizedVectorValues.iterator();
        }
