...
public class NativeEngineKnnVectorQuery extends Query {
    ...
    public Weight createWeight(IndexSearcher indexSearcher, ScoreMode scoreMode, float boost) throws IOException {
        ...
        if (isRescoreRequired(firstPassKFor2PhaseSearch) == false) {
            ...
            if (true && knnQuery.getCompressionLevel() == CompressionLevel.x32) {
                // Run BOTH rescore paths and compare
                // Save a copy of phase-1 results for the full-precision path
                List<PerLeafResult> phase1Copy = new ArrayList<>();
                for (PerLeafResult plr : perLeafResults) {
                    ScoreDoc[] origDocs = plr.getResult().scoreDocs;
                    ScoreDoc[] copy = new ScoreDoc[origDocs.length];
                    for (int ci = 0; ci < origDocs.length; ci++) {
                        copy[ci] = new ScoreDoc(origDocs[ci].doc, origDocs[ci].score);
                    }
                    TopDocs copyTd = new TopDocs(plr.getResult().totalHits, copy);
                    phase1Copy.add(new PerLeafResult(plr.getFilterBits(), plr.getFilterBitsCardinality(), copyTd, plr.getSearchMode()));
                }
                ...
                java.util.Map<Integer, Float> ecScoreMap = new java.util.HashMap<>();
                for (int si = 0; si < ecResults.size(); si++) {
                    int docBase = leafReaderContexts.get(si).docBase;
                    for (ScoreDoc sd : ecResults.get(si).getResult().scoreDocs) {
                        ecScoreMap.put(sd.doc + docBase, sd.score);
                    }
                }
                // Merge FP results across segments
                java.util.Map<Integer, Float> fpScoreMap = new java.util.HashMap<>();
                for (int si = 0; si < fpResults.size(); si++) {
                    int docBase = leafReaderContexts.get(si).docBase;
                    for (ScoreDoc sd : fpResults.get(si).getResult().scoreDocs) {
                        fpScoreMap.put(sd.doc + docBase, sd.score);
                    }
                }

                // Get top-k from each
                List<java.util.Map.Entry<Integer, Float>> ecSorted = new ArrayList<>(ecScoreMap.entrySet());
                ecSorted.sort((a, b) -> Float.compare(b.getValue(), a.getValue()));
                List<java.util.Map.Entry<Integer, Float>> fpSorted = new ArrayList<>(fpScoreMap.entrySet());
                fpSorted.sort((a, b) -> Float.compare(b.getValue(), a.getValue()));

                int topK = finalK;
                java.util.Set<Integer> ecTopK = new java.util.LinkedHashSet<>();
                for (int ti = 0; ti < Math.min(topK, ecSorted.size()); ti++) {
                    ecTopK.add(ecSorted.get(ti).getKey());
                }
                java.util.Set<Integer> fpTopK = new java.util.LinkedHashSet<>();
                for (int ti = 0; ti < Math.min(topK, fpSorted.size()); ti++) {
                    fpTopK.add(fpSorted.get(ti).getKey());
                }
